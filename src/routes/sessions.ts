...
export const createSessionsRoute: RouteCreator =
  (createHelpers) => async (req, res) => {
    ...
    let oidcCredentials = { provider: '', id_token: null as string | null, access_token: null as string | null, refresh_token: null as string | null }
    ...
    if (session?.identity?.id) {
      try {
        ...
        logger.debug('Retrieved identity with credentials', { identityId: session.identity.id })
        ...
        if (identityDetails.credentials?.oidc?.config) {
          ...
          const oidcConfig = identityDetails.credentials.oidc.config as { providers?: Array<{
            provider: string;
        ...
        logger.error('Error retrieving identity details', { error, identityId: session.identity.id })
      }
    }
    ...
    res.render("session", {
      ...
      oidcCredentials,
      hasOidcCredentials: Boolean(oidcCredentials.id_token || oidcCredentials.access_token || oidcCredentials.refresh_token),
      sessionCodeBox: CodeBox({
        className: "session-code-box",
        children: JSON.stringify(session, null, 2),
      }),
      // OIDC tokens code box if available
      oidcTokensCodeBox: oidcCredentials.id_token ? CodeBox({
